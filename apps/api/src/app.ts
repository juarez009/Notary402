...
function redactedUrl(raw?: string) {
  if (!raw) return null;
  try {
...
export function buildApp(options: AppOptions = {}) {
  ...
  app.post("/v1/payments/l402/verify", async (request, reply) => {
    ...
    const proof = { payment_proof_id: id("pay"), signature_request_id: String(body.signature_request_id), provider: "aperture" as const, network: "polar" as const, receipt: String(body.receipt), status: "verified" as const, created_at: now() };
    reply.code(201);
  ...
  app.post("/v1/payments/amoy/verify", async (request, reply) => {
    try {
      ...
      if ((error as Error).name === "AMOY_RPC_NOT_CONFIGURED") return reply.code(503).send({ error: "AMOY_RPC_NOT_CONFIGURED", message: "Set AMOY_RPC_URL", statusCode: 503 });
      throw error;
    }
  });
  ...
  app.post("/v1/attestations", async (request, reply) => {
    ...
    if (!walletProof) return reply.code(400).send({ error: "MissingWalletProof", message: "Wallet proof is required", statusCode: 400 });
    const paymentProof = await store.getPaymentProof(String(body.payment_proof_id));
    if (!paymentProof) return reply.code(400).send({ error: "MissingPaymentProof", message: "Payment proof is required", statusCode: 400 });
    ...
    const attestation = {
      ...
      status: analysis?.requires_human_notary ? "pending_human_review" as const : "issued" as const,
