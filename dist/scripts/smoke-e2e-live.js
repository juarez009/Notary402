...
function assertStatus(step, expected, response) {
    if (response.statusCode !== expected)
        throw new Error(`${step} failed: expected ${expected}, got ${response.statusCode} ${response.body}`);
}
const app = buildApp();
try {
    await app.ready();
    ...
    if (liveStatus.json().supabase?.configured !== true)
        throw new Error(`live status failed: supabase not configured ${liveStatus.body}`);
    ...
    if (analysis.requires_human_notary !== false)
        throw new Error(`legal analysis failed: expected low risk, got ${legalAnalysis.body}`);
    ...
    if (attestation.status !== "issued" || attestation.valid !== true)
        throw new Error(`attestation failed: expected issued/valid, got ${attestationResponse.statusCode} ${attestationResponse.body}`);
    ...
    const failedChecks = Object.entries(verification.checks).filter(([, passed]) => !passed).map(([name]) => name);
    if (verification.valid !== true || failedChecks.length)
        throw new Error(`verify failed: valid=${verification.valid}, failed checks [${failedChecks.join(", ")}] ${verifyResponse.body}`);
    ...
    const events = timeline.json().events;
    if (events.length < 6)
        throw new Error(`timeline failed: expected >= 6 events, got ${events.length} ${timeline.body}`);
    ...
    for (const required of ["legal_intent", "signature_request", "wallet_proof", "payment_proof", "legal_analysis", "attestation"]) {
        if (!steps.has(required))
            throw new Error(`timeline failed: missing step ${required} ${timeline.body}`);
    }
