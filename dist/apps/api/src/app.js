...
async function tcpReachable(hostPort, timeoutMs = 600) {
...
async function httpReachable(raw, timeoutMs = 900) {
    if (!raw)
        return false;
...
async function evmRpcReachable(raw, expectedChainId, timeoutMs = 1200) {
    if (!raw)
        return false;
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), timeoutMs);
